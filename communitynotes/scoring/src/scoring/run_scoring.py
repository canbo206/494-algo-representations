...
def run_prescoring(
  ...
  with c.time_block("Note Topic Assignment"):
    ...
    noteTopics = topicModel.get_note_topics(
      ...
      exitOnLowAccuracy=False,  # Disable validation for core-only mode
    )
...
def run_scoring(
  ...
  postSelectionSimilarityValues = pd.DataFrame(columns=[
      c.raterParticipantIdKey,  
      c.postSelectionValueKey,
      c.quasiCliqueValueKey,   
      c.modelingGroupKey
  ])
  logger.info("Skipped rater clustering for faster processing")
  (
    prescoringNoteModelOutput,
    prescoringRaterModelOutput,
  ...
  # Step 3: Only MFCoreScorer and prescoring. Skip final scoring and contributor scoring.
  # Return prescoring outputs so the runner writes the 4 output files.
  logger.info("Core-only mode: returning prescoring outputs (skipping final + contributor scoring).")
  minimal_aux = noteStatusHistory[[c.noteIdKey]].drop_duplicates()
  return (
    prescoringNoteModelOutput,
    prescoringRaterModelOutput,
    noteStatusHistory,
  ...
  # --- Commented out: final scoring and contributor scoring ---
